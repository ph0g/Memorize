...
struct MemoryGame<CardContent> where CardContent: Equatable {
    ...
    struct Card: Identifiable {
        ...
        // befor a certain amount of time passes during wich the card is face up
        ...
        // (I.e. not icnluding the current time it's been face up if it is currently so)
        ...
        // percentage if the bonus time remaining
        var bonusRemaining: Double {
            (bonusTimeLimit > 0 && bonusTimeRemaining > 0) ? bonusTimeRemaining/bonusTimeLimit : 0
        }
        
        // wether the card was matched during the bonus period
